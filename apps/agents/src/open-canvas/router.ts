...
function countContextDocuments(contextDocuments: CtxDoc[] = []) {
  ...
  for (const d of contextDocuments) {
    fileCount++;
    if (d.type === "code") loc += countLines(d.data);
    else loc += countWords(d.data);
  }
...
function conditionallyGenerateTitle(
  state: any
  ...
  const totalChars = (
    Array.isArray(state._messages) ? state._messages : []
  ).reduce((acc: number, msg: any) => {
    if (typeof msg?.content === "string") return acc + msg.content.length;
    const parts = Array.isArray(msg?.content) ? msg.content : [];
    const chars = parts.reduce(
      (a, c) => a + (typeof c?.text === "string" ? c.text.length : 0),
      0
    );
    return acc + chars;
  }, 0);
  const { fileCount, loc } = countContextDocuments(
